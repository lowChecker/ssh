...
echo "🚀 Starting Nullmailer installation and configuration..."
...
if [ $? -ne 0 ]; then
    echo "❌ Error: Nullmailer installation failed."
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Error: Nullmailer service restart failed."
    exit 1
fi

# 5. Verification and Test
echo "--- Nullmailer Configuration Complete ---"
echo "✅ Nullmailer configured to relay mail to: **${MAILRISE_HOST}:${MAILRISE_PORT}**"
echo ""
